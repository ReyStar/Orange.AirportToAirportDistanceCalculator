...
namespace Routes.API.Filters
{
    ...
    public class KnownExceptionFilter : ExceptionFilterAttribute
    {
        ...
        {
            if (context.Exception is RouteServiceException airportInformationProviderException)
            {
                WriteException(airportInformationProviderException, HttpStatusCode.InternalServerError, context);
            }
            else if (context.Exception is RouteNotFoundException airportNotFoundException)
            {
